...
And(
  "se em members nao existe nenhum membro com o email {string} escrito no campo email",
  (string) => {
    get("input")
      .invoke("val")
      .then((val1) => {
...
Then(
  "eu devo ver uma notificação no formulario de Senha Obrigatoria e o conteudo no campo da {string} tem que ser vazio",
  (id) => {
    the(id).should("contain", "");
  }
);
Then(
  "eu devo ver uma notificação no formulario de E-Mail Obrigatório e o conteudo no campo do {string} tem que ser vazio",
  (id) => {
    the(id).should("contain", "");
  }
);
...
And(
  "se em users existe algum user com o email {string} escrito no campo email",
  (string) => {
    get("input")
      .invoke("val")
      .then((val1) => {
...
And(
  "se em members existe algum member e nenhum user com o email {string} escrito no campo email",
  (string) => {
    get("input")
      .invoke("val")
      .then((val1) => {
